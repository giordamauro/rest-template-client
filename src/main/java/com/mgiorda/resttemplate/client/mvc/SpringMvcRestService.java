...
public class SpringMvcRestService<T, R> implements RestService<T, R>{
	...
	public SpringMvcRestService(MethodInvocation<T, R> methodInvacation){
		...
		Objects.requireNonNull(methodInvacation);
		
		this.methodInvacation = methodInvacation;
		this.mvcServiceData = new SpringMvcServiceData<>(methodInvacation.getInterfaceClass(), methodInvacation.getMethod(), methodInvacation.getArgs());
	}
	...
	public HttpMethod getHttpMethod() {
		...
		return mvcServiceData.getMethodHttpMethods()
				.map(httpMethods -> 
					httpMethods.get(0))
						.orElseGet(() -> 
							mvcServiceData.getClassHttpMethods()
	...
	public String getServiceUrl() {
		...
		String basePath = mvcServiceData.getClassUrls()
				.map(urls -> urls[0])
				.orElse("");
		
		String servicePath = mvcServiceData.getMethodUrls()
				.map(urls -> urls[0])
				.orElse("");
		
		final StringBuilder serviceUrl = new StringBuilder(basePath + servicePath);
		
		mvcServiceData.getParmetersForAnnotation(RequestParam.class)
	...
	public Class<R> getResponseType() {
		...
		Class<R> returnType = (Class<R>) methodInvacation.getMethod().getReturnType();
	...
	public Optional<HttpHeaders> getHeaders() {
		...
		mvcServiceData.getClassHeaders()
			.ifPresent(headers -> addHeaders(headers, httpHeaders));
		
		mvcServiceData.getMethodHeaders()
			.ifPresent(headers -> addHeaders(headers, httpHeaders));
		
		getContentTypeHeader()
			.ifPresent(contentType -> httpHeaders.add("Content-Type", contentType));
	...
	private Optional<String> getContentTypeHeader(){
		...
		Optional<String> consumesValue = mvcServiceData.getMethodConsumes()
				.filter(consumes -> consumes.length == 1)
				.map(consumes -> consumes[0]);
				
		if(!consumesValue.isPresent()){
			consumesValue = mvcServiceData.getClassConsumes()
					.filter(consumes -> consumes.length == 1)
					.map(consumes -> consumes[0]);
		}
